...
def key_pressed():
    cmd = str(keyboard.read_key()).lower()
    time.sleep(0.2)
    if 'up' in cmd:
        cmd = 'up'
    elif 'down' in cmd:
...
name = input("Who am I? Name: ").title()
player = Player(name)
print("Where am I?")
print("I can't see...")
print(player)
...
game_on = True
while game_on:
    print("What should I do? Action: ",end='')
    action = key_pressed().strip().upper()
    if action == "I":
    ...
    if player.get_location() in enemies.values():
        ...
        while enemy.get_health() > 0 and player.get_health_value() > 0:
            print("I can't move until I've defeated the enemy! What should I do? Action: ",end='')
            action = key_pressed().strip().upper()
            if "ATTACK" in action:
                print(player.use_item(action.split()[1].lower()))
